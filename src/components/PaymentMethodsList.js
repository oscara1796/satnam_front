  ...
  return (
    <div>
      ...
      <div className='instructions-container'>
        ...
        <p>
          Añade, configura y elimina métodos de pago fácilmente desde tu perfil.
          Selecciona tu metodo principal para transacciones futuras. Gestiona
          tus pagos con seguridad y conveniencia.
        </p>
      </div>
      ...
      {paymentMethods.all_payment_methods.length > 0 && (
        <div className='payment-methods-list'>
          {paymentMethods.all_payment_methods.map((method) => (
            <div
              key={method.id}
              className={`payment-method-box  ${defaultPaymentMethod && defaultPaymentMethod.id === method.id ? 'payment-method-selected' : ''}`}
            >
              ...
              <div className='payment-method-actions'>
                ...
                defaultPaymentMethod.id === method.id ? (
                  ...
                  <Button
                    variant='secondary'
                    size='sm'
                    onClick={() => applyDefaultPaymentMethod(method)}
                  >
                    Principal metodo de Pago
                  </Button>
                )}
      ...
      <Button onClick={addMethodsButton}>
        {showForm ? 'Cerrar formulario' : 'Agregar metodo de pago'}
      </Button>
