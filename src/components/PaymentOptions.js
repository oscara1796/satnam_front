...
const PaymentOptions = ({isLoggedIn}) => {
  ...
  const paymentOptions = [
    { id: 1, label: 'Tarjeta credito', component: <SubscriptionForm isLoggedIn={isLoggedIn}    />},
    { id: 2, label: 'PayPal' },
    ...
    <div className='container my-3 payments-box'>
      ...
      <div className="payment-options mt-2">
        {paymentOptions.map((option) => (
          <div key={option.id} className={`payment-option ${selectedOption === option.id ? 'selected' : ''}`}>
            <input
              type="checkbox"
              id={`option-${option.id}`}
      ...
      {selectedOption && (
        <div className="selected-option">
          {paymentOptions.find((option) => option.id === selectedOption).component}
        </div>
