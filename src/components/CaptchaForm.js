...
const CaptchaForm = (props) => {
    ...
    const handleSubmit = async (e, {resetForm}) => {
        e.preventDefault();
        try {
            const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/captcha/`, { captcha: captchaInput });
            ...
            if (response.status >= 200 && response.status < 300) {
                ...
                console.log('CAPTCHA validation failed with status code:', response.status);
            }
    ...
    return (
        <form onSubmit={handleSubmit} className='mt-3'>
            ...
            {props.captchaValidated && <div className="success-message">{successMessage}</div>}
        </form>
    );
};
